<template name="yq-avatar">
	<view class="room-content">
		<image :src="imgSrc.imgSrc" @click="fSelect" v-if="imgSrc.imgSrc" class="coverView"></image>
		<view v-else class="coverView" @click="fSelect">
			<slot>
				<uni-icons color="#aaaaaa" type="plusempty" size="40" />
			</slot>
		</view>
		...
		<view class="oper-wrapper" :style="{display: sD, top:tp}" v-if="footer">
			<view class="oper">
				<view class="btn-wrapper" v-if="sO">
					...
					<view @click="fUpload" hover-class="hover" :style="{width: bW}"><text>保存</text></view>
				</view>
...
<script>
	...
	export default {
		...
		beforeDestroy() {
			uni.$off("closeCrop", this.fClose)
			uni.$off("saveAvatar", this.fUpload)
			uni.$off("cropUserPhoto", this.cropUserPhoto)
		},
		created() {
			uni.$on("cropUserPhoto", this.cropUserPhoto)
			uni.$on("closeCrop", this.fClose)
			uni.$on("saveAvatar", this.fUpload)
		...
		methods: {
			fWindowResize() {
				...
				this.csH = this.wH - (this.footer ? tH : 0)  + 'px';
				// #endif
			...
			cropUserPhoto(img){
				...
				let path = this.imgPath = img;
				uni.getImageInfo({
					src: path,
					success: r => {
						this.$emit("changeTitle", 2)
			...
			fSelect() {
				if (this.fSelecting || this.disable) return;
				...
				uni.chooseImage({
					...
					success: async (r) => {
						...
						let tmp = await this.compressImageHandler(r.tempFilePaths[0])
						...
						uni.getImageInfo({
							...
							success: r => {
								...
								if(system_info.brand === 'samsung' && (r.orientation == 'right' || r.orientation == 'left')){
									this.imgWidth = r.height;
									this.imgHeight = r.width;
								} else {
									this.imgWidth = r.width;
									this.imgHeight = r.height;
								}
